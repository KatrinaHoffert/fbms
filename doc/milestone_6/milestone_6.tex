...
\todonote{Alsharif. Adapt from M4. Mention some of the checks in file change handlers.}
...
\todonote{Need 2 sequence diagrams per member.\\ Tao: FileOp.applyPatch() and FileOp.createPatch();\\ Hoffert: GuiController.revertRevision() and GuiController.viewRevision();\\ Butler: GuiController.changeLiveDirectory() and GuiController.change\-BackupDirectory();\\ Alsharif: FileHistory.getRevisionInfo() and FileHistory.storeRevision();\\ Rizvi: Startup.resolveBackupDirectory() and DbConnection.getConfig().\\ Don't spend too much time on this, it's worth very little marks.}

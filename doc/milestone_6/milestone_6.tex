...
\begin{description}
	...
	The file change handler will process the list: \\
	The system will either make a diff file or copy tdomaihe file to backup folder.
	\item \textbf{Quality} \\
	This is a main use case in the system and must work to a very high standard.
	\item \textbf{Version} \\
	26 November 2013	
...
The class diagram is too large to embed in this document, so has been included separately as the file \texttt{domain\_{}diagram.png}.
...
\includegraphics[width=\linewidth] {images/revert_revision_sequence.png}
...
\includegraphics[width=\textwidth]{images/changeLiveDirectory.png}

\subsubsection{GuiController.changeBackupDirectory}
\includegraphics[width=\textwidth]{images/changeBackupDirectory.png}
...
\begin{itemize}
	...
	\item Test cases \\
	The test file is in \texttt{/trunk/main/src/cmpt370/fbms/test}. All test cases are in package \texttt{cmpt370.fbms.test} to keep other packages clean.
	
	\item What is tested \\
	FileOp, FileChangeHandlers, and parts of DbConnection, DataRetriever is tested by JUnit. Some methods are tested in one test, since they have a close relationship. Some time-depended result needs human to check.\\
	The GUI parts (\texttt{Errors}) are test by a special \texttt{TestCase} class, TesterVisual. Since GUI needs "look good", this test case always reports successful. However, you need to check the look and feel of the windows.
	
	\item JUnit is helpful \\
...
\item The if clauses in \texttt{DbConnection} (line 115 and 129) and \texttt{Startup} (line 270) are reversed to enhance the readability.

\item The nested classes in \texttt{FirstStartWizard}, \texttt{RevisionDialog} and \texttt{MainFrame} are not implementing \texttt{MouseListener}. They are extending \texttt{MouseAdapter} to reduce code length.

\item Some excalmatory marks, \texttt{==true} in \texttt{FileChangeHandlers} are removed to enhance the readability.
...
\begin{itemize}
\item Copy the selected file to a chosen folder: From the ``File'' menu, choose ``copy-to'' while selecting a file. This will open a prompt for choosing the location you wish to copy the selected file to.
\item Restore all files: From the ``File'' menu, choose ``restore all''. This will open a prompt for choosing the location you wish to restore all the files to.
\item Change the live or backup directory: From the ``File'' menu, choose either ``Change live directory'' or `Change backup directory''. This will open a prompt for choosing the directory. The live and backup directories cannot be children of each other.
\end{itemize}
...
\begin{itemize}
\item A file is created: if the file doesn't already exist, it is merely backed up. If the file does exist, this is treated like a modification.
\item A file is modified: if the file already exists, it is backed up and revisioned. If it doesn't already exist, this is treated like a creation.
\item A file is renamed: if the old name already exists, that is renamed, otherwise this is treated like a creation (under the new name). If the new name doesn't already exist, we rename the file. If the new name already exists, we treat this like a modification, backing up and revisioning the file. The current behavior of ``renaming'' a file is to make a copy and move the revisions to the new name. To allow the old name to be restored, it is not deleted. Therefore, we end up with copies (this will be improved in future versions of the software, which will keep track of name history).
\item A file is deleted: The file is not removed from the backup directory, but files that no longer exist in the live directory will be marked with an exclamation mark in the file browser.
\end{itemize}
...
\begin{longtable}{| p{0.2cm} p{6.25cm} | p{3cm}| p{5cm} |}
  ...
  2. & Requirements and early design &  &  \\ \hline
   & 2.1. Summary use cases & Hoffert(2h)\newline Tao(1.5h)\newline Rizvi(2h) \newline Alsharif(2h) \newline Butler(2h) &  \\ \hline
   & 2.2. Fully-dressed use cases & Hoffert(0.5h) \newline Tao(0.5h)\newline Rizvi(1.5h) \newline Alsharif (1.5h)) \newline Butler(0.5h)& Adapted to current design. \\ \hline
   & 2.3. Use case diagram & Tao(0.5h) &  \\ \hline
   & 2.4. Domain model & Butler(3h) &  \\ \hline
   & 2.5. Glossary & Hoffert(0.25h) &  \\ \hline
   & 2.6. Supplementary specification & Hoffert(0.25h) \newline Tao(0.5h) \newline Rizvi(1h) \newline Alsharif(1.5h) \newline Butler(0.5h)& \\ \hline
   & 2.7. System sequence diagrams & Hoffert(0.5h)\newline Rizvi(1.5h) \newline Alsharif(1.5h) \newline Butler(0.5h)& \\ \hline
   & 2.8. Operation contracts & Hoffert(1.5h)\newline Tao(1.5h) & References of use cases by Tao \\ \hline
  ...
  5. & Complete implementation &  & These conventions were followed from the start \\ \hline
   ...
   & 5.5. Complete implementation & Hoffert(1h) &  \\ \hline
   & 5.6. User manual & Hoffert(0.5h) &  \\ \hline
  ...
  8. & Acknowledgements & Hoffert(0.25h) &  \\ \hline
   & References & Hoffert(0h) &  \\ \hline
    & \textbf{Total member contributions} & \textbf{Hoffert} & 15.5h \\ \cline{3-4}
    &  & \textbf{Tao} & 13.5h \\ \cline{3-4}
    &  & \textbf{Butler} & 11.5h \\ \cline{3-4}
    &  & \textbf{Rizvi} & 9.0h\\ \cline{3-4}
    &  & \textbf{Alsharif} & 10.0h\\ \hline
    & \textbf{Grand total} &  & 59.5h \\ \hline
\end{longtable}
...
\begin{longtable}{| p{2cm} | p{5cm} | p{3cm}| p{4.5cm} |}
  ...
  Hoffert & Planning and documentation & 5 hours & Almost entire technical details document\\ \cline{2-4}
   ...
   & Usability fixes & 3 hours & As suggested by marker in M5 \\ \hline
   & Miscellaneous bug fixes & 4 hours & Numerous smaller bug fixes\\ \hline
  \multicolumn{2}{|r|}{\textbf{Total for Hoffert}} & 96 hours & 62\% of the commits from one person\\ \hline \hline
  Tao & Rough design of modules & 1 hour & \\ \cline{2-4}
   ...
   & Revamping test case & 4 hours & \\ \cline{2-4}
   ...
   & Miscellaneous bug fixes & 3.5 hours & \\ \hline
  \multicolumn{2}{|r|}{\textbf{Total for Tao}}  & 51 hours & Preliminary design and library suggestion \\ \hline \hline
  Butler & Milestone 1-5 & 8 hours & \\ \cline{2-4}
   & getConfig() & 3 hours & \\ \cline{2-4}
   & setConfig & 3 hours & \\ \cline{2-4}
   & handleCreatedFiles & 6 hours & \\ \cline{2-4}
   & handleDeletedFiles & 2 hours & \\ \cline{2-4}
   & handleModifiedFiles & 4 hours & \\ \cline{2-4}
   & handleRenamedFiles & 3 hours & \\ \cline{2-4}
   &  Refactoring & 3.5 hours & File change handlers \\ \cline{2-4}
   & testing & 4 hours & \\ \cline{2-4}
   & debugging/optimization & 4 hours & \\ \hline 
  \multicolumn{2}{|r|}{\textbf{Total for Butler}}  & 40.5 hours & \\ \hline \hline
  Rizvi & Milestone(1-4) documentation & 7 hours  & \\ \cline{2-4}
   & frontend implementation  & 4 hours  & \\ \cline{2-4}
   &  GUI brainstorm & 7 hours & \\ \cline{2-4}
   &  GUI implementation & 10 hours & \\ \cline{2-4}
   &  Testing and reporting & 6 hours & \\ \cline{2-4}
   &  Milestone 5 & 3 hours & \\ \hline
  \multicolumn{2}{|r|}{\textbf{Total for Rizvi}}  &  37 hours & \\ \hline \hline
  Alsharif & Milestone (1-4)  & 9 hours & \\ \cline{2-4}
   &  work on presentation slides & 3 hours & \\ \cline{2-4}
   &  FileOp.creatPatch &  4 hours & \\ \cline{2-4}
   &  FileOp.fileToList &  4 hours & \\ \cline{2-4}
   &  Test FileOp.creatPatch &  2 hours & \\ \cline{2-4}
   &  Test FileOp.fileToList &  3 hours & \\ \cline{2-4}
   &  FileOp.applyPatch &  6 hours & \\ \cline{2-4}
   &  Milestone 5 &  4 hours & \\ \hline
   
  \multicolumn{2}{|r|}{\textbf{Total for Alsharif}}  & 35 hours & \\ \hline
\end{longtable}

...
\begin{enumerate}
	...
	26 November 2013	
\end{enumerate}
\item Change backup directory$^{Tao}$
\begin{enumerate}
	\item[•] Level \hfill \\
	Summary
	\item[•] User \hfill \\
...
\noindent{\includegraphics[width=\linewidth]{images/use_cases.png}} \\
...
\todonote{Delete files needs to be redone (Rizvi), rename folder needs to be more detailed (Alsharif); all of them will require renaming since the objectification}
...
\begin{description}
\item[Cross references:] \texttt{FileChangeHandlers.handleModifiedFiles()}, \texttt{FileChangeHandlers.} \texttt{handleRenamedFiles()}, \texttt{Startup.startupScan()}
\item[Precondition:] \texttt{oldRevision} and \texttt{currentRevision} exist
\item[Postconditions:] Creates a temporary file containing the patch to patch \texttt{currentRevision} to \texttt{oldRevision}.
\end{description}

\vspace{0.75cm}

\noindent{\textbf{\texttt{FileOp.applyPatch(Path currentRevision, Path patchFile)}}}
\begin{description}
\item[Cross references:] \texttt{FileHistory.displayRevision(Path, long)}
...
\noindent{\textbf{\texttt{DataRetriever.getFolderContents(Path folder)}}}
\begin{description}
...
\noindent{\textbf{\texttt{GuiController.displayRevision(Path file, long timestamp)}}}
\begin{description}
...
\noindent{\textbf{\texttt{GuiController.changeLiveDirectory(Path newDirectory)}}}
\begin{description}
...
\noindent{\textbf{\texttt{Startup.resolveBackupDirectory()}}}
\begin{description}
...
\noindent{\textbf{\texttt{DbManager.getFileRevisions(Path file)}}}
\begin{description}
...
\noindent{\textbf{\texttt{DbManager.setConfig(String settingName, String settingValue)}}}
\begin{description}
...
\noindent{\textbf{\texttt{FileOp.copy(Path file, Path destinationFolder)}}}
\begin{description}
...
\noindent{\textbf{\texttt{FileHistory.storeRevision(Path file, Path diff, long filesize, long delta)}}}
\begin{description}
\item[Cross references:] \texttt{FileChangeHandlers.handleModifiedFiles()}, \texttt{FileChangeHandlers.}
\texttt{handleRenamedFiles()}, \texttt{GuiController.revertRevision(Path, long)}
...
\todonote{Revise these. We only got a 7/10 here last time. What can we improve?}
...
\todonote{Need 2 sequence diagrams per member.\\ Da: FileOp.applyPatch() and FileOp.createPatch();\\ Mike: GuiController.revertRevision() and GuiController.viewRevision();\\ Michael: GuiController.changeLiveDirectory() and GuiController.change\-BackupDirectory();\\ Hattan: FileHistory.getRevisionInfo() and FileHistory.storeRevision();\\ Ahsen: Startup.resolveBackupDirectory() and DbConnection.getConfig().\\ Don't spend too much time on this, it's worth very little marks.}
...
\todonote{Everyone needs to point out at least one instance of refactoring, file and lines, why we did it, etc. Take a look at my objectification changes from revision 436 to 448 if you need ideas.}
...
\todonote{Add the release and development folders (last minute)}
...
\todonote{Test this manual on someone outside our group}
...
\noindent{\textbf{Some code derived from work by:}}
\begin{itemize}
...
\noindent{\textbf{FBMS is licensed under GPL v3:}} \\
\url{https://code.google.com/p/fbms} \\

\noindent{\textbf{Log4j is licensed under the Apache License v2:}} \\
\url{https://logging.apache.org/log4j/1.2/} \\

\noindent{\textbf{SQLite JDBC Driver is licensed under the Apache License v2:}} \\
\url{https://bitbucket.org/xerial/sqlite-jdbc} \\

\noindent{\textbf{JNotify is licensed under the LGPL v2:}} \\
\url{http://jnotify.sourceforge.net/} \\

\noindent{\textbf{google-diff-match-patch is licensed under the Apache License v2:}} \\
\url{https://code.google.com/p/google-diff-match-patch/} \\

\noindent{\textbf{Java MIME Magic is licensed under GPL v2:}} \\
